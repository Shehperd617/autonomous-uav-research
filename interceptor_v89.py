# ============================================================
# DRONE INTERCEPTOR v8.9 -- UNLOCKED SPEED
# SITL speed limits removed via custom parm file
# [+] fast_interceptor.parm loaded at SITL boot
# [+] SITL --defaults loads BOTH default + fast params
# [+] Verified groundspeed printed every tick
# [+] ProNav + anti-evasion hold
...
parser = argparse.ArgumentParser(
    description="Drone Interceptor v8.9")
parser.add_argument(
    "--connect",
...
parser.add_argument(
    ...
    help="Path to base defaults file")
parser.add_argument(
    "--fast-params",
    default=os.path.expanduser(
        "~/drone_interceptor/fast_interceptor.parm"),
    help="Path to fast interceptor params")
parser.add_argument(
    "--test-physics",
    action="store_true",
...
class Config:
    ...
    SAFE_ZONE       = 15
    LOOP_RATE       = 0.1
    COMBAT_LOOP     = 0.05   # faster updates in combat
    MAX_RADIUS      = 300
    HEARTBEAT_LIMIT = 2.0
    JAM_RANGE       = 50
    JAM_DURATION    = 1.5
    JAM_SUCCESS     = 0.40
    RAM_TRIGGER     = 10.0   # LARGE trigger -- at speed we blow through this
    TRACK_SPEED     = 15     # closing speed
    RAM_SPEED       = 30     # intercept pursuit speed
    TERMINAL_SPEED  = 40     # final kill speed -- no mercy
    TERMINAL_DIST   = 15     # switch to terminal inside this
    BERSERK_TIME    = 30     # seconds before going berserk
...
class State:
    ...
    JAM       = "JAM"
    INTERCEPT = "INTERCEPT"
    TERMINAL  = "TERMINAL"   # inside 15m -- full send
    BERSERK   = "BERSERK"    # timeout -- straight line max speed
    BRAKE     = "BRAKE"
    RTL       = "RTL"
    COMPLETE  = "COMPLETE"
...
def start_sitl():
    ...
    if not os.path.exists(args.sitl_path):
        log_err(f"SITL not found: "
               f"{args.sitl_path}")
        sys.exit(1)
    # Check for fast params file
    if not os.path.exists(args.fast_params):
        log_warn(f"Fast params not found: {args.fast_params}")
        log_warn("Creating fast_interceptor.parm...")
        parm_dir = os.path.dirname(args.fast_params)
        os.makedirs(parm_dir, exist_ok=True)
        with open(args.fast_params, "w") as f:
            f.write("WPNAV_SPEED          3000\n")
            f.write("WPNAV_SPEED_DN       1500\n")
            f.write("WPNAV_ACCEL          800\n")
            f.write("WPNAV_ACCEL_Z        500\n")
            f.write("ANGLE_MAX            6000\n")
            f.write("PILOT_SPEED_UP       5000\n")
            f.write("PILOT_SPEED_DN       3000\n")
            f.write("LOIT_SPEED           3000\n")
            f.write("LOIT_ACC_MAX         800\n")
            f.write("PSC_VELXY_P          3.0\n")
            f.write("PSC_POSZ_P           3.0\n")
            f.write("ARMING_CHECK         0\n")
            f.write("SIM_GPS_NUMSATS      15\n")
        log_ok(f"Created: {args.fast_params}")
    # Load BOTH default AND fast params (comma separated)
    combined_defaults = f"{args.defaults},{args.fast_params}"
    log_info(f"Starting SITL with FAST params...")
    log_info(f"  Defaults: {combined_defaults}")
    ...
    log_ok("SITL started with FAST params!")
    time.sleep(3)
...
def verify_speed_params():
    """Verify fast params loaded correctly at SITL boot."""
    log_info("Verifying speed parameters...")
    checks = {
        "WPNAV_SPEED": 3000,
        "ANGLE_MAX": 6000,
        "WPNAV_ACCEL": 800,
    ...
    for name, expected in checks.items():
        try:
            actual = vehicle.parameters[name]
            ok = abs(actual - expected) < 1
            status = "OK" if ok else "WRONG"
            if not ok:
                all_ok = False
                # Try to set it
                vehicle.parameters[name] = expected
                time.sleep(0.3)
            log_info(f"  {name}: {actual} (want {expected}) [{status}]")
        except Exception as e:
            log_warn(f"  {name}: {e}")
            all_ok = False
    if all_ok:
        log_ok("All speed params verified!")
    else:
        log_warn("Some params needed correction")
    # Print actual max speed for reference
    try:
        wpnav = vehicle.parameters["WPNAV_SPEED"]
        log_info(f"  MAX NAV SPEED: {wpnav/100:.0f} m/s")
    except:
        pass

# ============================================================
# PRONAV GUIDANCE (v8.9)
# Instead of chasing, calculate the collision point
# and fly there. When target evades, HOLD BEARING
# for 1 second -- the target flies across our path.
# ============================================================
class ProNavGuide:
    """
    Proportional Navigation guidance.
    Keeps the line-of-sight rate at zero --
    this naturally creates a collision course.
    """
    def __init__(self):
        self.prev_los = None
        self.prev_time = None
        self.hold_bearing = None
        self.hold_until = 0
        self.nav_gain = 4.0  # PN gain (3-5 typical)

    def update(self, my_pos, target, distance):
        now = time.time()

        # Calculate line-of-sight angle
        los = get_bearing(my_pos, target.position())

        # If target just started evading, HOLD current bearing
        # The target turns 90 deg -- if we hold straight,
        # it crosses our flight path
        if target.evading and self.hold_bearing is None:
            self.hold_bearing = los
            self.hold_until = now + 1.0  # hold for 1 second

        # If holding bearing (anti-evasion)
        if self.hold_bearing is not None and now < self.hold_until:
            return self.hold_bearing

        # Release hold
        if now >= self.hold_until:
            self.hold_bearing = None

        # ProNav: steer to zero out LOS rate
        if self.prev_los is not None and self.prev_time is not None:
            dt = max(now - self.prev_time, 0.001)
            # LOS rate in deg/s
            los_rate = los - self.prev_los
            ...
            # PN correction: steer perpendicular to LOS
            # This naturally creates an intercept course
            correction = self.nav_gain * los_rate * dt

            # Apply correction to bearing
            cmd_bearing = (los + correction) % 360
        else:
            # First update -- fly directly at target
            cmd_bearing = los

        self.prev_los = los
        self.prev_time = now

        # For close range (< 20m), go direct -- no time for PN
        if distance < 20:
            return los
...
# ============================================================
# INTERCEPT PHYSICS MODULE (NEW in v8.9)
#
...
def engage(target, all_targets):
    ...
    while (target.active and
        ...
        # v8.9: Update ALL active targets every tick
        ...
        # -- INTERCEPT (v8.9 -- ProNav guidance) --
        elif state == State.INTERCEPT:

            if near_vip(target.lat, target.lon):
                log_safe("Near VIP! Re-attempting jam!")
                state          = State.JAM
                jam_start_time = time.time()
                continue

            # ProNav calculates optimal bearing
            cmd_bearing = pronav.update(my_pos, target, distance)

            # Altitude matching
            alt_diff = target.alt - my_pos.alt
            vd = 0
            if abs(alt_diff) > 0.5:
                vd = -max(-5.0, min(5.0, alt_diff))

            # Speed: ramp up as we get closer
            if distance > 30:
                spd = Config.RAM_SPEED
            else:
                spd = Config.TERMINAL_SPEED

            # FLY! No deceleration, no hesitation
            fly_toward_bearing(cmd_bearing, spd, vd)

            # Physics assessment
            my_heading = get_bearing(my_pos, target.position())
            assessment = InterceptPhysics.evaluate(
                my_pos=my_pos,
                my_heading=cmd_bearing,
                my_speed=spd,
            ...
            hold = "HOLD" if pronav.hold_bearing is not None else "PN"
            phase = "TERM" if spd == Config.TERMINAL_SPEED else "CLOS"
            actual_gs = vehicle.groundspeed or 0
            ...
            print(
                ...
                f"Dist:{C.YELLOW}"
                f"{distance:.1f}m{C.END} | "
                f"cmd:{spd} gs:{actual_gs:.0f}m/s | "
                f"KE:{assessment['ke_effective']:.0f}J | "
                f"{hold} | "
                f"Brg:{cmd_bearing:.0f}")

            # KILL CHECK
            if distance < Config.RAM_TRIGGER:
                ke = assessment["ke_effective"]
                v_close = assessment["closing_speed"]
                impact = assessment["impact_type"]
                print(
                    f"\n{C.RED}{C.BOLD}"
                    f"{'='*45}\n"
        ...
        # Use faster loop rate in combat
        if state in [State.INTERCEPT]:
            time.sleep(Config.COMBAT_LOOP)
        else:
...
if __name__ == "__main__":
    ...
    print(f"\n{C.CYAN}{'='*45}{C.END}")
    print(f"{C.BOLD}"
          f"{'INTERCEPTOR v8.9':^45}"
          f"{C.END}")
    print(f"{C.CYAN}"
          f"{'UNLOCKED | ProNav | Fast Params':^45}"
          f"{C.END}")
    print(f"{C.CYAN}{'='*45}{C.END}\n")
    ...
    print(f"\n{C.CYAN}"
          f"INTERCEPTOR v8.9 ONLINE{C.END}")
    print(f"{C.CYAN}"
          f"UNLOCKED | ProNav | Priority | "
          f"Evasion Predict{C.END}\n")
    time.sleep(1)

    # v8.9: PRIORITY TARGETING LOOP

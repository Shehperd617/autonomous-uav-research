#!/usr/bin/env python3
"""
SPEAR Monte Carlo v5.2 — bigger terminal boost, extended terminal phase, full lead.
Modes: passive | easy | medium | hard
Flags: --noise, --strict, --ekf, --apn
"""
import numpy as np
...
TGT_BURST_SPD=22.0
INT_SPD=22.0
INT_SPD_TERMINAL=40.0       # was 30 — bigger overtake margin against burst evaders
HIT_DIST_NORMAL=3.0
HIT_DIST_STRICT=1.5
N_PN=4.5
N_APN=4.5
TERMINAL_R=18.0             # was 12 — more time in high-speed terminal phase
LEAD_GAIN=0.8
LEAD_GAIN_TERMINAL=1.0      # full lead in terminal phase
DT=0.05
MAX_SIM_TIME=30.0
...
class TargetEKF:
    """
    6-state Kalman filter, fixed Q, soft innovation gate.
    Exposes acceleration via heavily smoothed and clipped finite difference.
    """
    def __init__(self):
        self.x=np.zeros(6)
...
class ProNav3D:
    """
    PN/APN + lead pursuit with extended terminal phase and big speed boost.
    APN only fires when tgo is short (close to intercept).
    """
    def __init__(self, use_ekf=False, use_apn=False):
...
def run_engagement(seed, mode, noise, hit_dist, use_ekf, use_apn):
    ...
    nav=ProNav3D(use_ekf=use_ekf, use_apn=use_apn)
...
def main():
    ...
    print("="*60)
    print(f"  SPEAR Monte Carlo v5.2 — {args.runs} engagements [{label}]")
    ...
    for i,seed in enumerate(seeds):
        r=run_engagement(int(seed),args.mode,args.noise,hit_dist,args.ekf,args.apn)
        results.append(r)
        if not args.quiet:

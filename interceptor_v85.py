# ============================================================
# DRONE INTERCEPTOR v8.5
# Updated from v8.5 with:
# [+] FLY-THROUGH intercept - aims PAST target, not at it
# [+] Predictive lead during ram phase
# [+] MAVLink velocity commands for no-decel ramming
# [+] Increased RAM_TRIGGER from 3m to 5m
# [+] RAM_SPEED boosted to 20 m/s
# [+] KE uses commanded speed, not groundspeed
# [+] Timeout extended to 180s for intercept phase
# ------------------------------------------------------------
# Carried forward:
...
parser = argparse.ArgumentParser(
    description="Drone Interceptor v8.5")
parser.add_argument(
    "--connect",
...
# ============================================================
# INTERCEPT PHYSICS MODULE (NEW in v8.5)
#
...
# ============================================================
# ENGAGE - TRUE FSM (v8.5)
# Fly-through ramming with velocity commands
# ============================================================
def engage(target):
    global MISSION_FAILED
    ...
    while (target.active and
        ...
        # -- INTERCEPT (Fly-Through Ram - v8.5) --
...
if __name__ == "__main__":
    ...
    print(f"\n{C.CYAN}{'='*45}{C.END}")
    print(f"{C.BOLD}"
          f"{'INTERCEPTOR v8.5':^45}"
          f"{C.END}")
    print(f"{C.CYAN}"
          f"{'Fly-Through Ram | Velocity Cmd | Physics':^45}"
          f"{C.END}")
    print(f"{C.CYAN}{'='*45}{C.END}\n")
    ...
    print(f"\n{C.CYAN}"
          f"INTERCEPTOR v8.5 ONLINE{C.END}")
    print(f"{C.CYAN}"
          f"Fly-Through | Velocity | Heartbeat | "

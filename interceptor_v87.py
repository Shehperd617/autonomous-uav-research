# ============================================================
# DRONE INTERCEPTOR v8.7 -- FAST KILL
# Updated from v8.7 with:
# [+] 3-phase speed: TRACK 15 / INTERCEPT 30 / TERMINAL 40 m/s
# [+] TERMINAL state inside 15m -- max speed, no hesitation
# [+] Evasion prediction -- cuts the corner, doesnt follow
# [+] Aggression timer -- 30s in INTERCEPT triggers BERSERK
# [+] Faster loop rate 0.05s in combat phases
# [+] RAM_TRIGGER increased to 6m for high-speed fly-through
# [+] ArduPilot speed params pushed via MAVLink
# ------------------------------------------------------------
# Carried forward:
# [+] Priority targeting + target switching
# [+] Fly-through ramming with MAVLink velocity commands
# [+] InterceptPhysics module
# [+] All failsafes (heartbeat/geofence/battery)
# ============================================================
...
parser = argparse.ArgumentParser(
    description="Drone Interceptor v8.7")
parser.add_argument(
    "--connect",
...
class Config:
    ...
    JAM_DURATION    = 1.5
    JAM_SUCCESS     = 0.40
    RAM_TRIGGER     = 6.0    # larger trigger for high speed
...
def push_speed_params():
    """Push aggressive speed parameters to ArduPilot."""
    log_info("Pushing speed parameters...")
    try:
        vehicle.parameters["WPNAV_SPEED"] = 3000      # 30 m/s
        vehicle.parameters["WPNAV_ACCEL"] = 400        # 4 m/s^2
        vehicle.parameters["ANGLE_MAX"] = 5000          # 50 deg max lean
        vehicle.parameters["PILOT_SPEED_UP"] = 5000     # 50 m/s up
        log_ok("Speed params set: WPNAV=30m/s ANGLE=50deg")
    except Exception as e:
        log_warn(f"Param push partial: {e}")

# ============================================================
# EVASION PREDICTOR (NEW in v8.7)
# When target evades, predict where it will go
# and cut the corner instead of following
# ============================================================
def predict_evasion(my_pos, target):
    """
    When target starts evading, it turns 90 degrees
    from the approach bearing. We predict which side
    and aim to cut it off.
    """
    bearing_to_target = get_bearing(my_pos, target.position())

    # Target evades perpendicular to our approach
    # Predict it will continue its current evasion heading
    # Aim ahead of where its going, not where it is
    if target.evading and target.speed_ms() > 0:
        # Target evasion velocity in m/s
        t_speed = target.speed_ms()
        # Predict 1.5 seconds ahead of evasion
        lead_time = 1.5
        pred_lat = target.lat + target.dlat * (lead_time / Config.COMBAT_LOOP)
        pred_lon = target.lon + target.dlon * (lead_time / Config.COMBAT_LOOP)
        return pred_lat, pred_lon

    # Not evading -- aim at predicted intercept point
    return predict_intercept(my_pos, target,
                            Config.RAM_SPEED, target.dt)
...
# ============================================================
# INTERCEPT PHYSICS MODULE (NEW in v8.7)
#
...
def engage(target, all_targets):
    ...
    while (target.active and
        ...
        # v8.7: Update ALL active targets every tick
        ...
        # -- INTERCEPT (v8.7 -- pursuit phase) --
            ...
            # Check for BERSERK timeout
            intercept_elapsed = time.time() - start
            if (intercept_elapsed > Config.BERSERK_TIME and
                    state == State.INTERCEPT):
                log_threat(
                    f"BERSERK MODE! {intercept_elapsed:.0f}s "
                    f"elapsed -- MAX AGGRESSION")
                state = State.BERSERK
                flight_log("STATE_CHANGE",
                           {"state": State.BERSERK})
                continue
            ...
            if distance < Config.TERMINAL_DIST:
                log_state(
                    f"INTERCEPT -> TERMINAL | "
                    f"{distance:.0f}m | FULL SEND")
                state = State.TERMINAL
                flight_log("STATE_CHANGE",
                           {"state": State.TERMINAL})
                continue

            # v8.7: EVASION-AWARE pursuit
            pred_lat, pred_lon = predict_evasion(
                my_pos, target)
            ...
            evd = "EVD" if target.evading else "---"
            ...
            print(
                ...
                f"{assessment['impact_type']} | "
                f"KE:{assessment['ke_effective']:.0f}J | "
                f"{evd}")

            # Kill check at RAM_TRIGGER
            if distance < Config.RAM_TRIGGER:
                ...
                print(
                    ...
                    f"   Speed:   {Config.RAM_SPEED} m/s\n"
                    f"   Mass:    {Config.INTERCEPTOR_MASS} kg\n"
                    f"   Kill:    {'YES' if assessment['kill_likely'] else 'UNCERTAIN'}\n"
                    f"{'='*45}"
                    f"{C.END}\n")
                target.active = False
        ...
        # -- TERMINAL (v8.7 -- inside 15m, full send) --
        elif state == State.TERMINAL:

            # No VIP check -- we are committed to kill
            # Aim directly at target, maximum speed
            ram_bearing = get_bearing(my_pos, target.position())
            ...
            fly_toward_bearing(ram_bearing,
                              Config.TERMINAL_SPEED, vd)

            assessment = InterceptPhysics.evaluate(
                my_pos=my_pos,
                my_heading=ram_bearing,
                my_speed=Config.TERMINAL_SPEED,
                target_pos=target.position(),
                target_heading=target.heading,
                target_speed=target.speed_ms(),
                interceptor_mass=Config.INTERCEPTOR_MASS)
            ...
            print(
                f"{C.RED}{C.BOLD}[TERMINAL]{C.END} | "
                f"T{target.id} | "
                f"Dist:{C.RED}"
                f"{distance:.1f}m{C.END} | "
                f"{C.RED}{C.BOLD}"
                f"{Config.TERMINAL_SPEED}m/s{C.END} | "
                f"KE:{assessment['ke_effective']:.0f}J")

            # If target escapes terminal range, back to intercept
            if distance > Config.TERMINAL_DIST * 1.5:
                log_state("TERMINAL -> INTERCEPT | target escaped close range")
                state = State.INTERCEPT
                continue
            ...
            if distance < Config.RAM_TRIGGER:
                ...
                print(
                    ...
                    f"   Close V: {v_close} m/s\n"
                    f"   KE:      {ke:.0f} J effective\n"
                    f"   Speed:   {Config.TERMINAL_SPEED} m/s\n"
                    f"   Mass:    {Config.INTERCEPTOR_MASS} kg\n"
                    f"   Kill:    CONFIRMED\n"
                    f"{'='*45}"
                    f"{C.END}\n")
        ...
        # -- BERSERK (v8.7 -- timeout, straight line kill) --
        elif state == State.BERSERK:

            # No evasion prediction, no angle optimization
            # Straight at target, maximum possible speed
            ram_bearing = get_bearing(my_pos, target.position())
            ...
            print(
                f"{C.RED}{C.BOLD}[BERSERK]{C.END} | "
                f"T{target.id} | "
                f"Dist:{C.RED}"
                f"{distance:.1f}m{C.END} | "
                f"{C.RED}{C.BOLD}"
                f"{Config.TERMINAL_SPEED}m/s "
                f"STRAIGHT LINE{C.END} | "
                f"KE:{assessment['ke_effective']:.0f}J")
        ...
        if state in [State.INTERCEPT, State.TERMINAL, State.BERSERK]:
...
if __name__ == "__main__":
    ...
    print(f"\n{C.CYAN}{'='*45}{C.END}")
    print(f"{C.BOLD}"
          f"{'INTERCEPTOR v8.7':^45}"
          f"{C.END}")
    print(f"{C.CYAN}"
          f"{'FAST KILL | 3-Phase | Evasion Predict':^45}"
          f"{C.END}")
    print(f"{C.CYAN}{'='*45}{C.END}\n")
    ...
    print(f"\n{C.CYAN}"
          f"INTERCEPTOR v8.7 ONLINE{C.END}")
    print(f"{C.CYAN}"
          f"FAST KILL | 3-Phase | Priority | "
          f"Evasion Predict{C.END}\n")
    time.sleep(1)

    # v8.7: PRIORITY TARGETING LOOP
